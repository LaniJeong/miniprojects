...
class Dino:         # 공룡 클래스
    ...
    def duck(self):
        self.image = self.duck_img[self.step_index // 5]     # 10 0, 1
    ...
    def jump(self):
        ...
        if self.jump_vel < -self.JUMP_VEL:  # -9.0이 되면 점프 중단
            self.dino_jump = False
            self.jump_vel = self.JUMP_VEL   # 9.0으로 초기화0
...
class SamllCactus(Obstacle):
    def __init__(self, image) -> None:
        self.type = random.randint(0, 2)    # 작은 선인장 세개 중 하나를 고름
        super().__init__(image, self.type)
        self.rect.y = 325
...
def main():
    ...
    while run:
        ...
        dino.draw(SCREEN)       # 공룡 그리기  공룡이 구름보다 먼저 그려져야함
        ...
        if len(obstacles) == 0:
            if random.randint(0, 2) == 0:
                obstacles.append(SamllCactus(SMALL_CACTUS))
